...
public:
    ...
    /// Was a local variable automatically mapped to another variable?
...
struct lvar_ref_t
{
    ...
    ///       are specific to a mbl_array_t object and cannot migrate between
...
#define OPTI_NO_LDXOPT 0x0008 ///< do not optimize low/high(ldx)
  //@}
...
#define MBA2_ALL_FLAGS    0x000007FF
...
struct cdg_insn_iterator_t
{
    ...
    bool is_separate_dslot; // the current insn is the separate delay slot
                            // insn (when the delay slot starts a block)
...
public:
    ...
    codegen_t(mbl_array_t* m) : mba(m), mb(NULL), ignore_micro(IM_NONE) {}
...
public:
    ...
    /// Save user-defined labels into the database
    void save_user_labels(void) const { ::save_user_labels2(entry_ea, user_labels, this); }
    /// Save user-defined comments into the database
...
struct ui_stroff_applicator_t
{
    ...
    /// \param path   path describing the union selection
    virtual bool idaapi apply(size_t opnum, const intvec_t& path) = 0;
};
...
//--------------------------------------------------------------------------
inline mblock_t* mbl_array_t::copy_block(mblock_t* blk, int new_serial, int cpblk_flags)
{
    return (mblock_t*)hexdsp(hx_mbl_array_t_copy_block, this, blk, new_serial, cpblk_flags);
}
...
//--------------------------------------------------------------------------
inline void mbl_array_t::serialize(bytevec_t& vout) const
{
    hexdsp(hx_mbl_array_t_serialize, this, &vout);
}
